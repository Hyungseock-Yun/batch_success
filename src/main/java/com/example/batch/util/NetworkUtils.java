    ...
    try {
      ...
      while (nienum.hasMoreElements()) {
        ...
        while (kk.hasMoreElements()) {
          ...
          if (inetAddress.getHostAddress().indexOf("119.205.238.") > -1) {
            return "real";
          } else if (inetAddress.getHostAddress().indexOf("119.205.238.") > -1) {
            return "script";
          }
          // 112.175.76.xxx 은 kakao
          else if (inetAddress.getHostAddress().indexOf("112.175.76.") > -1) {
            return "kakao";
          } else if (inetAddress.getHostAddress().indexOf("119.205.215.") > -1) {
            return "dev";
          } else if (inetAddress.getHostAddress().indexOf("192.168.100") > -1) {
            return "local";
          }
        }
      }
    ...
    try {
      ...
      while (nienum.hasMoreElements()) {
        ...
        while (kk.hasMoreElements()) {
          InetAddress inetAddress = (InetAddress) kk.nextElement();
          // 로컬 테스트시 주석 해제
          if (inetAddress.getHostAddress().indexOf("192.168.100") > -1) {
            log.debug("getHostAddress" + inetAddress.getHostAddress());
            bool = true;
            break;
          }
          for (String serverIp : abServerList) {
            if (inetAddress.getHostAddress().indexOf("119.205.238." + serverIp) > -1) {
              bool = true;
              break;
            }
          }
